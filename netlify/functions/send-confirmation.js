...
// ── row() MUST be defined at module scope, before the handler uses it ────────
...
exports.handler = async (event) => {
  ...
  const { order_no, name, email, phone, payment, ticket_type, referral_code, amount } = body;
  ...
  const paymentLabel = payment === "credit_card" ? "信用卡線上刷卡" : "現金轉帳／匯款";
  const isKol = ticket_type === "kol" && referral_code;
  const ticketLabel = isKol ? `KOL 優惠票（推薦碼：${referral_code}）` : "標準票";
  ...
  // ── row() is now safely in scope above ──────────────────────────────────────
...
<table width="600" cellpadding="0" cellspacing="0"
       style="max-width:600px;width:100%;background:#0E0B05;border-radius:4px;overflow:hidden;border:1px solid #C9A84C;">
  ...
  <tr>
    <td style="padding:32px 40px;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0"
             style="background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.22);border-radius:4px;margin-bottom:20px;">
        <tr>
      ...
      <!-- REMINDERS -->
      <div style="background:rgba(255,255,255,.03);border-left:3px solid #C9A84C;padding:14px 18px;margin:20px 0;border-radius:0 4px 4px 0;">
  ...
  <tr>
    <td style="background:#0A0700;padding:20px 40px;text-align:center;border-top:1px solid rgba(201,168,76,.15);">
      <p style="margin:0;color:rgba(250,246,238,.3);font-size:11px;line-height:2;letter-spacing:1px;">
        富豐企管顧問有限公司<br>此為系統自動發送，請勿直接回覆此信件
      </p>
    </td>
  </tr>
  ...
  // ── Send via Resend ──────────────────────────────────────────────────────────
  // IMPORTANT: `from` must match a domain verified in your Resend dashboard.
  // Gmail addresses (@gmail.com) are blocked by Resend — use your own domain.
  try {
    ...
    if (!resendRes.ok) {
      // Full Resend error visible in Netlify function logs
    ...
    console.log("Confirmation email sent OK — id:", resendData.id, "to:", email);
    return {
      statusCode: 200,
